...
> **Security note:** `env-files` is for _application_ configuration only. Because these files are part of the checked-out repository, a pull request could otherwise execute code before evaluation, redirect inherited provider credentials or evaluation data, bypass the failure gate, or redirect cache cleanup. The action therefore rejects process controls — Node/npm (`NODE_OPTIONS`, `NODE_PATH`, `NPM_CONFIG_*`), git (`GIT_*`), interpreter startup/search paths and executable selection (`PYTHONPATH`, `PYTHONUSERBASE`, `RUBYLIB`, `PERL5LIB`, `PROMPTFOO_PYTHON`, `PROMPTFOO_RUBY`, ...), executable resolution (`PATH`), dynamic loaders (`LD_*`, including `LD_AUDIT`, and `DYLD_*`), config-home redirection (`HOME`, `XDG_CONFIG_HOME`, ...), proxy/TLS controls (`HTTP_PROXY`, `NODE_TLS_REJECT_UNAUTHORIZED`, `PROMPTFOO_INSECURE_SSL`, ...), provider/cloud/telemetry endpoints and routing controls (`OPENAI_BASE_URL`, `ANTHROPIC_BASE_URL`, `API_HOST`, `AWS_ENDPOINT_URL`, `AWS_ENDPOINT_URL_*`, `CLOUDFLARE_GATEWAY_ID`, `PROMPTFOO_CLOUD_API_URL`, `PROMPTFOO_REMOTE_GENERATION_URL`, `OTEL_EXPORTER_OTLP_ENDPOINT`, ...), and Promptfoo action controls (`PROMPTFOO_CACHE_PATH`, `PROMPTFOO_CONFIG_DIR`, `PROMPTFOO_PASS_RATE_THRESHOLD`, `PROMPTFOO_FAILED_TEST_EXIT_CODE`, ...) — and fails the run if any selected file sets one. Ordinary application variables (`NODE_ENV`, custom provider settings, API keys, ...) still pass through. If your evaluation genuinely needs a protected override (for example, a provider endpoint, outbound proxy, custom CA, or Python module path), set it in the trusted workflow `env:` block instead of an `env-files` file:
>
> ```yaml
>       - name: Run promptfoo evaluation
>         uses: promptfoo/promptfoo-action@v1
