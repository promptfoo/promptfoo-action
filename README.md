...
> **Security note:** Promptfoo loads `working-directory/.env` (or `.env.vault` when a trusted `DOTENV_KEY` is configured) implicitly even when `env-files` is empty. The action validates that file first without overwriting trusted workflow values, then loads selected `env-files` in order. Repository environment files are for _application_ configuration only: they must not execute code before evaluation, redirect inherited credentials or evaluation data, change the failure gate, or redirect cache cleanup.
>
> When an evaluation is needed, the action also preflights `commandLineOptions.envPath` in YAML or JSON configs, including local `$ref` targets and in-workspace symlinks. Config globs, executable JavaScript or TypeScript configs, computed/template env paths, additional implicit `promptfooconfig.*` or `redteam.*` configs, and paths or refs that escape the working directory are rejected because they cannot be safely preflighted. Use a single static config and literal repository-local paths, or move application environment files to the action's `env-files` input.
>
> The action rejects Node/npm/git, interpreter/Bundler, Go/compiler, browser, and dynamic-loader controls (for example `NODE_DEBUG`, `NODE_GYP_FORCE_PYTHON`, `PYTHON`, `NPM_CONFIG_*`, `RUBYGEMS_GEMDEPS`, `BUNDLE_*`, `GOFLAGS`, `GOCACHEPROG`, `PLAYWRIGHT_BROWSERS_PATH`, `LD_*`), along with dotenv-vault/preload controls (`DOTENV_KEY`, `DOTENV_CONFIG_*`). It also rejects Promptfoo authentication (`PROMPTFOO_API_KEY`, `PROMPTFOO_REMOTE_API_BASE_URL`), credential/config/metadata routing (`AWS_BEARER_TOKEN_BEDROCK`, `AWS_BEDROCK_REGION`, `AWS_REGION`, `AWS_DEFAULT_REGION`, `AWS_CONFIG_FILE`, `AWS_PROFILE`, `AWS_LOGIN_CACHE_DIRECTORY`, `AWS_WEB_IDENTITY_TOKEN_FILE`, `GOOGLE_APPLICATION_CREDENTIALS`, `GOOGLE_EXTERNAL_ACCOUNT_ALLOW_EXECUTABLES`, `GOOGLE_API_CERTIFICATE_CONFIG`, `GOOGLE_GENAI_USE_VERTEXAI`, `GOOGLE_CLOUD_PROJECT`, `GOOGLE_PROJECT_ID`, `GOOGLE_CLOUD_QUOTA_PROJECT`, `CLOUDSDK_*`, `AZURE_POD_IDENTITY_AUTHORITY_HOST`, `AZURE_STORAGE_CONNECTION_STRING`, `AZURE_TOKEN_CREDENTIALS`, `AZURE_TOKEN_SCOPE`, `AZURE_TENANT_ID`, `AZURE_REGIONAL_AUTHORITY_NAME`, `AZURE_ADDITIONALLY_ALLOWED_TENANTS`, `IDENTITY_ENDPOINT`, `IMDS_ENDPOINT`, `MSI_ENDPOINT`, `GCE_METADATA_HOST`, `GCE_METADATA_IP`, `OPENAI_ORGANIZATION`, `OPENAI_ORG_ID`, `OPENAI_PROJECT_ID`, `VERTEX_PROJECT_ID`, `SHAREPOINT_BASE_URL`, `SHAREPOINT_CERT_PATH`, `SHAREPOINT_CLIENT_ID`, `SHAREPOINT_TENANT_ID`, `CODEX_HOME`, ...), provider/cloud/TLS endpoints (`OPENAI_BASE_URL`, `ANTHROPIC_BASE_URL`, `AWS_ENDPOINT_URL_*`, `CDP_DOMAIN`, `NODE_TLS_REJECT_UNAUTHORIZED`, `GOOGLE_LOCATION`, `GOOGLE_CLOUD_LOCATION`, `VERTEX_REGION`, ...), and action-gate controls (`PROMPTFOO_CACHE_PATH`, `PROMPTFOO_CACHE_TTL`, `PROMPTFOO_CACHE_MAX_SIZE`, `PROMPTFOO_CACHE_MAX_FILE_COUNT`, `PROMPTFOO_PASS_RATE_THRESHOLD`, `PROMPTFOO_DISABLE_SHARING`, `PROMPTFOO_DISABLE_VAR_EXPANSION`, `PROMPTFOO_DISABLE_TEMPLATE_ENV_VARS`, `PROMPTFOO_DISABLE_CONVERSATION_VAR`, `PROMPTFOO_DISABLE_OBJECT_STRINGIFY`, `PROMPTFOO_SELF_HOSTED`, `PROMPTFOO_STRIP_*`, ...). The run fails if any implicit or selected file sets one. Ordinary application variables (`NODE_ENV`, custom provider settings, provider API keys such as `OPENAI_API_KEY`, ...) still pass through. If an evaluation genuinely needs a protected override, configure it in the trusted workflow `env:` block instead of an environment file:
>
> ```yaml
>       - name: Run promptfoo evaluation
>         uses: promptfoo/promptfoo-action@v1
