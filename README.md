...
> **Security note:** Promptfoo loads `working-directory/.env` implicitly even when `env-files` is empty, so the action validates that file first and then loads selected `env-files` in order. These files are for _application_ configuration only: a pull request could otherwise execute code before evaluation, redirect inherited provider credentials or evaluation data, bypass the failure gate, or redirect cache cleanup. The action therefore rejects process controls — Node/npm (`NODE_OPTIONS`, `NODE_PATH`, `NODE_DEBUG`, `NODE_DEBUG_NATIVE`, `NPM_CONFIG_*`), git (`GIT_*`), interpreter startup/search paths and executable selection (`PYTHONPATH`, `PYTHONUSERBASE`, `RUBYLIB`, `RUBYGEMS_GEMDEPS`, `GEM_HOME`, `GEM_PATH`, `BUNDLE_*`, `PERL5LIB`, `PROMPTFOO_PYTHON`, `PROMPTFOO_RUBY`, ...), Go compiler/toolchain/module controls (`GOFLAGS`, `GOENV`, `GOCACHEPROG`, `GOTOOLCHAIN`, `GOPROXY`, `GOSUMDB`, `GOINSECURE`, `CC`, `AR`, `GCCGO`, `CGO_*`, ...), browser executable/cache/download controls (`PLAYWRIGHT_BROWSERS_PATH`, `PLAYWRIGHT_*_DOWNLOAD_HOST`, `PUPPETEER_EXECUTABLE_PATH`, ...), executable/config resolution (`PATH`, `HOME`, `XDG_CONFIG_HOME`, `AWS_CONFIG_FILE`, `AWS_SHARED_CREDENTIALS_FILE`, `AWS_PROFILE`, `AWS_WEB_IDENTITY_TOKEN_FILE`, `GOOGLE_APPLICATION_CREDENTIALS`, `GOOGLE_EXTERNAL_ACCOUNT_ALLOW_EXECUTABLES`, `CLOUDSDK_*`, `CODEX_HOME`, `CLAUDE_CONFIG_DIR`, `OPENCODE_CONFIG`, `OPENCODE_CONFIG_DIR`, ...), dynamic loaders (`LD_*`, including `LD_AUDIT`, and `DYLD_*`), proxy/TLS controls (`HTTP_PROXY`, `NODE_TLS_REJECT_UNAUTHORIZED`, `PROMPTFOO_INSECURE_SSL`, `AWS_CA_BUNDLE`, `CURL_CA_BUNDLE`, `REQUESTS_CA_BUNDLE`, ...), provider/cloud/telemetry endpoints and routing controls (`OPENAI_BASE_URL`, `ANTHROPIC_BASE_URL`, `AZURE_AI_PROJECT_URL`, `API_HOST`, `AWS_ENDPOINT_URL`, `AWS_ENDPOINT_URL_*`, `CDP_DOMAIN`, `CLOUDFLARE_GATEWAY_ID`, `PROMPTFOO_CLOUD_API_URL`, `PROMPTFOO_REMOTE_API_BASE_URL`, `PROMPTFOO_REMOTE_GENERATION_URL`, `OTEL_EXPORTER_OTLP_ENDPOINT`, ...), and Promptfoo action controls (`PROMPTFOO_CACHE_PATH`, `PROMPTFOO_CONFIG_DIR`, `PROMPTFOO_PASS_RATE_THRESHOLD`, `PROMPTFOO_FAILED_TEST_EXIT_CODE`, `PROMPTFOO_DISABLE_VAR_EXPANSION`, `PROMPTFOO_DISABLE_REF_PARSER`, `PROMPTFOO_DISABLE_TEMPLATING`, `PROMPTFOO_STRICT_FILES`, ...) — and fails the run if any implicit or selected file sets one. Ordinary application variables (`NODE_ENV`, custom provider settings, API keys, ...) still pass through. If your evaluation genuinely needs a protected override (for example, a provider endpoint, outbound proxy, custom CA, or Python module path), set it in the trusted workflow `env:` block instead of an `env-files` file:
>
> ```yaml
>       - name: Run promptfoo evaluation
>         uses: promptfoo/promptfoo-action@v1
