...
>
> The action rejects Node/npm/git, interpreter/Bundler, Go/compiler, browser, and dynamic-loader controls (for example `NODE_DEBUG`, `NPM_CONFIG_*`, `RUBYGEMS_GEMDEPS`, `BUNDLE_*`, `GOFLAGS`, `GOCACHEPROG`, `PLAYWRIGHT_BROWSERS_PATH`, `LD_*`), along with dotenv-vault/preload controls (`DOTENV_KEY`, `DOTENV_CONFIG_*`). It also rejects credential/config/metadata routing (`AWS_BEARER_TOKEN_BEDROCK`, `AWS_CONFIG_FILE`, `AWS_PROFILE`, `AWS_LOGIN_CACHE_DIRECTORY`, `AWS_WEB_IDENTITY_TOKEN_FILE`, `GOOGLE_APPLICATION_CREDENTIALS`, `GOOGLE_EXTERNAL_ACCOUNT_ALLOW_EXECUTABLES`, `GOOGLE_API_CERTIFICATE_CONFIG`, `CLOUDSDK_*`, `AZURE_POD_IDENTITY_AUTHORITY_HOST`, `AZURE_STORAGE_CONNECTION_STRING`, `AZURE_TOKEN_CREDENTIALS`, `AZURE_ADDITIONALLY_ALLOWED_TENANTS`, `IDENTITY_ENDPOINT`, `IMDS_ENDPOINT`, `MSI_ENDPOINT`, `GCE_METADATA_HOST`, `GCE_METADATA_IP`, `CODEX_HOME`, ...), provider/cloud/TLS endpoints (`OPENAI_BASE_URL`, `ANTHROPIC_BASE_URL`, `AWS_ENDPOINT_URL_*`, `CDP_DOMAIN`, `NODE_TLS_REJECT_UNAUTHORIZED`, `GOOGLE_LOCATION`, `GOOGLE_CLOUD_LOCATION`, `VERTEX_REGION`, ...), and action-gate controls (`PROMPTFOO_CACHE_PATH`, `PROMPTFOO_PASS_RATE_THRESHOLD`, `PROMPTFOO_DISABLE_VAR_EXPANSION`, `PROMPTFOO_DISABLE_TEMPLATE_ENV_VARS`, `PROMPTFOO_DISABLE_CONVERSATION_VAR`, `PROMPTFOO_DISABLE_OBJECT_STRINGIFY`, `PROMPTFOO_SELF_HOSTED`, ...). The run fails if any implicit or selected file sets one. Ordinary application variables (`NODE_ENV`, custom provider settings, API keys, ...) still pass through. If an evaluation genuinely needs a protected override, configure it in the trusted workflow `env:` block instead of an environment file:
>
> ```yaml
>       - name: Run promptfoo evaluation
>         uses: promptfoo/promptfoo-action@v1
