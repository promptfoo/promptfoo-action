...
| `env-files` | Comma-separated application `.env` paths, resolved from and restricted to `working-directory`, loaded in order. Later files override earlier files; traversal, escaping symlinks, process-control, credential-routing, and Promptfoo authentication variables are rejected. | No |
