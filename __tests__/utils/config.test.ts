...
describe('extractFileDependencies', () => {
  ...
  const openedContents = new Map<number, Buffer>();
  ...
  beforeEach(() => {
    ...
    mockFs.openSync.mockImplementation((value: string) => {
      const fileDescriptor = nextFileDescriptor++;
      openedPaths.set(fileDescriptor, value);
      return fileDescriptor;
    });
    mockFs.fstatSync.mockImplementation((fileDescriptor: number) =>
      mockFs.statSync(openedPaths.get(fileDescriptor)),
    );
    mockFs.readSync.mockImplementation(
      (
        ...
        let contents = openedContents.get(fileDescriptor);
        if (!contents) {
          contents = Buffer.from(
            String(
              mockFs.readFileSync(openedPaths.get(fileDescriptor), 'utf8'),
            ),
          );
          openedContents.set(fileDescriptor, contents);
        }
        const bytesRead = Math.min(length, contents.length - position);
        if (bytesRead > 0) {
          contents.copy(buffer, offset, position, position + bytesRead);
        }
        return Math.max(bytesRead, 0);
      },
    );
    mockFs.closeSync.mockImplementation((fileDescriptor: number) => {
      openedPaths.delete(fileDescriptor);
      openedContents.delete(fileDescriptor);
    });
  });

  it.each([
    ['FIFO or device', false, 0],
    ['oversized file', true, 10 * 1024 * 1024 + 1],
  ])('should not read a primary config that is an %s', (_kind, isFile, size) => {
    mockFs.statSync.mockReturnValue({
      isDirectory: () => false,
      isFile: () => isFile,
      size,
    ...
    mockFs.readFileSync.mockImplementation(() => {
      throw new Error('unsafe primary config was read');
    });
  ...
  it('should fail closed without reading a primary config that cannot be statted', () => {
    mockFs.statSync.mockImplementation(() => {
      throw new Error('EACCES: denied\n::error::forged-primary-stat');
    });
    ...
    expect(
      (core.warning as unknown as Mock).mock.calls
        ...
        .join('\n'),
    ).not.toContain('::error::forged-primary-stat');
  });

  it('should open a replaced primary config in nonblocking mode', () => {
    mockFs.readFileSync.mockImplementation(() => {
      throw new Error(
        'EAGAIN: FIFO replaced config\n::error::forged-primary-race',
      );
    });
  ...
  it('should fail closed when a primary config cannot be opened', () => {
    mockFs.openSync.mockImplementation(() => {
      throw new Error('EACCES: denied\n::error::forged-primary-open');
    });
    ...
    expect(
      (core.warning as unknown as Mock).mock.calls
        .map((call) => String(call[0]))
        .join('\n'),
    ).not.toContain('::error::forged-primary-open');
  });

  it('should fail closed when a primary config grows after stat', () => {
    mockFs.readFileSync.mockReturnValue('a'.repeat(10 * 1024 * 1024 + 1));
  ...
  it('should not open a primary config whose parent resolves outside the checkout', () => {
    mockFs.realpathSync.mockImplementation((value: string) =>
      value === '/test/working/promptfooconfig.yaml'
        ? '/private/tmp/outside/promptfooconfig.yaml'
        : value,
    );
  ...
  it('should preserve an in-checkout symlink to the original config file', () => {
    mockFs.realpathSync.mockImplementation((value: string) =>
      value === '/test/working/promptfooconfig.yaml'
        ? '/test/working/configs/original.yaml'
        : value,
    );
    mockFs.readFileSync.mockImplementation((value: string) =>
      value === '/test/working/configs/original.yaml'
        ? 'providers:\n  - file://providers/original.py'
        : '',
    );

    const deps = extractFileDependencies('/test/working/promptfooconfig.yaml');

    expect(deps).toEqual(['providers/original.py', './']);
    expect(mockFs.openSync).toHaveBeenCalledWith(
      '/test/working/configs/original.yaml',
      fs.constants.O_RDONLY | fs.constants.O_NONBLOCK | fs.constants.O_NOFOLLOW,
    );
  ...
  it('should not read a primary config replaced between stat and open', () => {
    mockFs.statSync.mockReturnValue({
      isDirectory: () => false,
      isFile: () => true,
      size: 0,
  ...
  it.each([
    ...
    'mtimeMs',
    'ctimeMs',
  ] as const)('should not read a primary config whose %s changes before open', (changedField) => {
    const beforeOpen = {
      isDirectory: () => false,
      isFile: () => true,
      size: 0,
    ...
    mockFs.statSync.mockReturnValue(beforeOpen);
    mockFs.fstatSync.mockReturnValue({
      ...beforeOpen,
      [changedField]: changedField === 'size' ? 1 : 11,
    } as fs.Stats);
  ...
  it.each([
    ...
    'mtimeMs',
    'ctimeMs',
  ] as const)('should not read a primary config whose %s changes after open', (changedField) => {
    const openedFile = {
      isDirectory: () => false,
      isFile: () => true,
      size: 0,
    ...
    mockFs.statSync.mockReturnValueOnce(openedFile).mockReturnValueOnce({
      ...openedFile,
      [changedField]: changedField === 'size' ? 1 : 11,
    } as fs.Stats);
    mockFs.fstatSync.mockReturnValue(openedFile);
  ...
  it.each([
    ...
    'mtimeMs',
    'ctimeMs',
  ] as const)('should not parse a primary config whose %s changes during the read', (changedField) => {
    const beforeRead = {
      isDirectory: () => false,
      isFile: () => true,
      size: 0,
    ...
    mockFs.statSync.mockReturnValue(beforeRead);
    mockFs.fstatSync.mockReturnValueOnce(beforeRead).mockReturnValueOnce({
      ...beforeRead,
      [changedField]: changedField === 'size' ? 1 : 11,
    } as fs.Stats);
  ...
  it('should not read a primary config after its parent is swapped before stat', () => {
    let parentSwapped = false;
    mockFs.realpathSync.mockImplementation((value: string) =>
      value === '/test/working/promptfooconfig.yaml' && parentSwapped
        ? '/private/tmp/outside/promptfooconfig.yaml'
        : value,
    );
    mockFs.statSync.mockImplementation((value: string) => {
  ...
  it('should not read a primary config replaced after open', () => {
    let configStats = 0;
    mockFs.statSync.mockImplementation((value: string) => {
      if (value === '/test/working/promptfooconfig.yaml') {
        configStats++;
...
`);
  ...
  it('should keep direct TLS, signature, and multipart data paths precise without executable auth hooks', () => {
    mockFs.readFileSync.mockReturnValue(`
providers:
  - id: https
    config:
...
`);
  ...
  it('should conservatively watch side inputs of a default-export HTTP auth hook', () => {
    mockFs.readFileSync.mockReturnValue(`
providers:
  - id: https
    config:
      auth:
        type: file
        path: auth/default.js
`);

    const deps = extractFileDependencies('/test/working/promptfooconfig.yaml');

    expect(deps).toEqual(['auth/default.js', './']);
    expect(core.warning).toHaveBeenCalledWith(
      expect.stringContaining('Watching the repository workspace'),
    );
  });
...
`);
  ...
  it.each([
    ['missing child', 'ENOENT'],
    ['non-directory child', 'ENOTDIR'],
  ])('should reject an escaped dependency glob prefix with a %s before enumeration', (_kind, code) => {
    mockFs.readFileSync.mockReturnValue(
      "tests: 'file://shared-link/missing/*.yaml'",
    );
    mockGlob.hasMagic.mockImplementation((value: string) =>
      value.includes('*'),
    );
    mockGlob.sync.mockImplementation(() => {
      throw new Error('escaped dependency glob prefix was enumerated');
    });
    mockFs.realpathSync.mockImplementation((value: string) => {
      if (value.endsWith('/shared-link/missing')) {
        throw Object.assign(new Error('not found'), { code });
      }
      return value.endsWith('/shared-link') ? '/test/outside' : value;
    });
  ...
  it('should sanitize a denied dependency glob prefix before enumeration', () => {
    mockFs.readFileSync.mockReturnValue(
      "tests: 'file://shared-link/missing/*.yaml'",
    );
    mockGlob.hasMagic.mockImplementation((value: string) =>
    ...
    mockGlob.sync.mockImplementation(() => {
      throw new Error('denied dependency glob prefix was enumerated');
    });
    mockFs.realpathSync.mockImplementation((value: string) => {
      if (value.endsWith('/shared-link/missing')) {
        throw Object.assign(
          new Error('denied\n::error::forged-glob-annotation'),
          { code: 'EACCES' },
        );
      }
      return value;
    ...
    expect(
      (core.warning as unknown as Mock).mock.calls
        .map((call) => String(call[0]))
        .join('\n'),
    ).not.toContain('::error::forged-glob-annotation');
  });
  ...
  ])('should not enumerate a pre-magic %s dependency traversal', (_kind, segment) => {
    mockFs.readFileSync.mockReturnValue(
      `extensions:\n  - 'file://evals/${segment}/${segment}/outside/*.js'`,
    );
    mockGlob.hasMagic.mockImplementation((value: string) =>
      value.includes('*'),
    );
    mockGlob.sync.mockImplementation(() => {
      throw new Error('pre-magic dependency traversal was enumerated');
    });
  ...
  it.each([
    ...
    ['reversed character class', '.[.]'],
    ['adjacent braces', '{.,.}{.,.}'],
    ['single brace', '{..,src}'],
    ['mixed braces', '{.,x}{.,y}'],
    ['class brace', '[.]{.,x}'],
    ['brace class', '{.,x}[.]'],
    ['leading negated extglob', '!(safe)'],
    ['negated extglob', '!(safe)'],
  ])('should not enumerate a %s dependency traversal segment', async (kind, segment) => {
    const realGlob = await vi.importActual<typeof import('glob')>('glob');
    mockFs.readFileSync.mockReturnValue(
      kind === 'leading negated extglob'
        ? `tests: 'file://tests/${segment}/*.yaml'`
        : `tests: 'file://tests/**/${segment}/outside/*.yaml'`,
    );
    mockGlob.hasMagic.mockImplementation(realGlob.hasMagic);
    mockGlob.sync.mockImplementation((pattern: string) => {
      if (
        ...
        throw new Error('unsafe dependency traversal was enumerated');
      }
      return [];
    });
    ...
    expect(deps.every((dependency) => dependency === 'tests/')).toBe(true);
    expect(core.warning).toHaveBeenCalledWith(
      expect.stringContaining('Ignoring unsafe config dependency glob pattern'),
    );
  });
...
providers:
  - id: https
    config:
      ...
      transformRequest: 'file://transforms/literal\\*.TS:request'
      transformResponse: 'file://transforms/literal\\[response].MJS:response'
      responseParser: 'file://transforms/literal\\parser.CTS:parse'
      sessionParser: 'file://transforms/literal\\session.MTS:session'
      session:
        responseParser: 'file://transforms/literal\\session-response.JS:parse'
      auth:
        type: file
...
`);
    ...
    expect(deps).toEqual([
      ...
      'validators\\STATUS\\{one,two}.JS:validate',
      'transforms/literal\\*.TS',
      'transforms/literal\\*.TS:request',
      'transforms/literal\\[response].MJS',
      'transforms/literal\\[response].MJS:response',
      'transforms/literal\\parser.CTS',
      'transforms/literal\\parser.CTS:parse',
      'transforms/literal\\session.MTS',
      'transforms/literal\\session.MTS:session',
      'transforms/literal\\session-response.JS',
      'transforms/literal\\session-response.JS:parse',
      'auth/token\\{literal}.py',
  ...
  it('should inspect nested dependencies from every provider YAML glob match', async () => {
    const realGlob = await vi.importActual<typeof import('glob')>('glob');
    const files = new Map([
      [
        '/test/working/promptfooconfig.yaml',
      ...
      [
        ...
        '- id: https\n  config:\n    tls:\n      caPath: certs/one.bundle',
      ],
      [
        '/test/working/providers/two.yaml',
        '- id: https\n  config:\n    signatureAuth:\n      privateKeyPath: signing/two.material',
      ],
    ]);
    mockFs.readFileSync.mockImplementation((value: string) => {
      const contents = files.get(value);
    ...
    expect(deps).toEqual([
      ...
      'providers/',
      'certs/one.bundle',
      'signing/two.material',
    ]);
    expect(core.warning).not.toHaveBeenCalled();
  });

  it('should inspect nested executable provider IDs from external provider configs', () => {
    const files = new Map([
      [
        '/test/working/promptfooconfig.yaml',
      ...
      [
        ...
        'id: file://providers/nested.yaml',
      ],
      [
        '/test/working/providers/nested.yaml',
        'id: python:custom/provider.py:call_api',
      ],
    ]);
    mockFs.readFileSync.mockImplementation((value: string) => {
      const contents = files.get(value);
  ...
  it('should track a root-level extensionless executable prompt', () => {
    mockFs.readFileSync.mockReturnValue('prompts: [exec:build_prompt]');
  ...
  it('should conservatively watch side inputs of path-shaped extensionless executable prompts', () => {
    mockFs.readFileSync.mockReturnValue('prompts: [prompts/build_prompt]');

    const deps = extractFileDependencies('/test/working/promptfooconfig.yaml');

    expect(deps).toEqual(['prompts/build_prompt', './']);
    expect(core.warning).toHaveBeenCalledWith(
      expect.stringContaining('Watching the repository workspace'),
    );
  });

  it('should track bare and file-backed providers and conservatively watch executable side inputs', () => {
    mockFs.readFileSync.mockReturnValue(`
providers:
  - bare-root.js
  - providers/bare.js
  - id: providers/object.ts
    config: {}
  - file://providers/file.py:call_api
  - ruby:providers/script.rb:call_api
extensions:
...
`);
  ...
  it('should conservatively watch the workspace for nested config refs', () => {
    mockFs.readFileSync.mockReturnValue(`
prompts:
  $ref: '#/shared/prompts'
shared:
...
tests:
  - provider:
      'https://example.test':
        config:
          tls:
            caPath: certs/per-test.bundle
          signatureAuth:
            privateKeyPath: signing/per-test.material
`);
  ...
  it('should rebase a map-style file-backed provider from an external test', () => {
    const files = new Map([
      ['/test/working/promptfooconfig.yaml', 'tests: file://tests/case.yaml'],
      [
        '/test/working/tests/case.yaml',
        `- provider:
  ...
  it('should preserve a runtime-loaded nested HTTP body file with literal glob syntax', async () => {
    const realGlob = await vi.importActual<typeof import('glob')>('glob');
    const files = new Map([
      [
        '/test/working/promptfooconfig.yaml',
...
`);
  ...
  it('should expand an optimized opening-brace character class in a config prompt path', async () => {
    const realGlob = await vi.importActual<typeof import('glob')>('glob');
    mockFs.readFileSync.mockReturnValue("prompts: 'file://prompts/[{].txt'");
    mockGlob.hasMagic.mockImplementation(realGlob.hasMagic);
    mockGlob.sync.mockReturnValue(['/test/working/prompts/{.txt']);

    const deps = extractFileDependencies('/test/working/promptfooconfig.yaml');

    expect(realGlob.hasMagic('prompts/[{].txt')).toBe(false);
    expect(deps).toEqual(['prompts/{.txt', 'prompts/']);
    expect(mockGlob.sync).toHaveBeenCalledWith(
      '/test/working/prompts/[{].txt',
...
`);
  ...
  it.each([
    ...
    ['unclosed class', 'file://providers/[safe/*.py'],
    ...
    [
      ...
      `file://providers/${'@(x)'.repeat(15_000)}.py`,
    ],
    [
      'excessive extglob depth',
      `file://providers/${'@('.repeat(65)}x${')'.repeat(65)}.py`,
    ],
  ...
  it('should fail closed before enumerating an excessive backslash-separated test glob', () => {
    mockFs.readFileSync.mockReturnValue(
      "tests: 'file://tests/\\{1..5000000}.yaml'",
    );
...
`);
  ...
  it('should track root-level and path-shaped CSV prompts without widening the workspace', () => {
    mockFs.readFileSync.mockReturnValue(
      'prompts: [cases.csv, prompts/cases.csv]',
    );

    const deps = extractFileDependencies('/test/working/promptfooconfig.yaml');

    expect(deps).toEqual(['cases.csv', 'prompts/cases.csv']);
    expect(core.warning).not.toHaveBeenCalled();
  });

  it('should ignore templated inline prose containing a slash without widening the workspace', () => {
    mockFs.readFileSync.mockReturnValue(
      "prompts: ['Summarize {{ input }} / explain the tradeoffs']",
    );
...
`);
  ...
  it('should not read a FIFO or device structured dependency', () => {
    mockFs.readFileSync.mockImplementation((value: string) => {
      if (value === '/test/working/promptfooconfig.yaml') {
        return 'prompts:\n  - file: prompts/blocked.yaml';
      }
      throw new Error('non-regular structured dependency was read');
    });
    mockFs.statSync.mockImplementation(
      (value: string) =>
        ({
          isDirectory: () => false,
          isFile: () => !value.endsWith('/blocked.yaml'),
          size: 0,
        }) as fs.Stats,
    );
  ...
  it('should open a replaced structured dependency in nonblocking mode', () => {
    mockFs.statSync.mockReturnValue({
      isDirectory: () => false,
      isFile: () => true,
      size: 0,
    ...
    mockFs.readFileSync.mockImplementation((value: string) => {
      if (value === '/test/working/promptfooconfig.yaml') {
        return 'prompts:\n  - file: prompts/replaced.yaml';
      }
      throw new Error(
        'EAGAIN: FIFO replaced dependency\n::error::forged-structured-race',
      );
    });

    const deps = extractFileDependencies('/test/working/promptfooconfig.yaml');

    expect(deps).toEqual(['prompts/replaced.yaml', './']);
    expect(mockFs.openSync).toHaveBeenCalledWith(
      '/test/working/prompts/replaced.yaml',
      fs.constants.O_RDONLY | fs.constants.O_NONBLOCK | fs.constants.O_NOFOLLOW,
    );
    expect(core.warning).toHaveBeenCalledWith(
      expect.stringContaining('Watching the repository workspace'),
  ...
  it('should not parse a structured dependency that grows after stat', () => {
    mockFs.statSync.mockReturnValue({
      isDirectory: () => false,
      isFile: () => true,
      size: 0,
    ...
    mockFs.readFileSync.mockImplementation((value: string) =>
      value === '/test/working/promptfooconfig.yaml'
        ? 'prompts:\n  - file: prompts/grown.yaml'
        : 'a'.repeat(10 * 1024 * 1024 + 1),
    );

    const deps = extractFileDependencies('/test/working/promptfooconfig.yaml');

    expect(deps).toEqual(['prompts/grown.yaml', './']);
    expect(core.warning).toHaveBeenCalledWith(
      expect.stringContaining('Watching the repository workspace'),
    );
  });

  it('should not open a structured dependency whose parent resolves outside the checkout', () => {
    mockFs.statSync.mockReturnValue({
      isDirectory: () => false,
      isFile: () => true,
      size: 0,
    ...
    mockFs.readFileSync.mockImplementation((value: string) =>
      value === '/test/working/promptfooconfig.yaml'
        ? 'prompts:\n  - file: prompts/swapped.yaml'
        : '',
    );
    mockFs.realpathSync.mockImplementation((value: string) =>
      value === '/test/working/prompts/swapped.yaml'
        ? '/private/tmp/outside/swapped.yaml'
        : value,
    );

    const deps = extractFileDependencies('/test/working/promptfooconfig.yaml');

    expect(deps).toEqual(['prompts/swapped.yaml', './']);
    expect(mockFs.openSync).not.toHaveBeenCalledWith(
      '/private/tmp/outside/swapped.yaml',
      expect.any(Number),
    );
    expect(mockFs.readFileSync).toHaveBeenCalledTimes(1);
  ...
  it('should not read a structured dependency replaced between stat and open', () => {
    mockFs.statSync.mockReturnValue({
      isDirectory: () => false,
      isFile: () => true,
      size: 0,
    ...
    mockFs.fstatSync.mockImplementation((fileDescriptor: number) => {
      const openedPath = openedPaths.get(fileDescriptor);
      return {
        isDirectory: () => false,
        isFile: () => true,
        size: 0,
        dev: 1,
        ino: openedPath?.endsWith('/swapped.yaml') ? 2 : 1,
      } as fs.Stats;
    });
    mockFs.readFileSync.mockImplementation((value: string) =>
      value === '/test/working/promptfooconfig.yaml'
  ...
  it.each([
    ...
    'mtimeMs',
    'ctimeMs',
  ] as const)('should not parse a structured dependency whose %s changes during the read', (changedField) => {
    const configPath = '/test/working/promptfooconfig.yaml';
    const dependencyPath = '/test/working/prompts/swapped.yaml';
    const configContents = 'prompts:\n  - file: prompts/swapped.yaml';
    const dependencyContents = 'value: safe';
    const snapshots = new Map([
      ...
      [
        dependencyPath,
        {
          isDirectory: () => false,
          isFile: () => true,
          size: Buffer.byteLength(dependencyContents),
          dev: 1,
          ino: 2,
          mtimeMs: 10,
          ctimeMs: 10,
    ...
    const dependencyFstats = new Map<number, number>();
    mockFs.statSync.mockImplementation((value: string) => snapshots.get(value));
    mockFs.fstatSync.mockImplementation((fileDescriptor: number) => {
      const openedPath = openedPaths.get(fileDescriptor);
      const snapshot = openedPath ? snapshots.get(openedPath) : undefined;
      if (!snapshot || openedPath !== dependencyPath) {
        return snapshot;
      }
      const calls = (dependencyFstats.get(fileDescriptor) ?? 0) + 1;
      dependencyFstats.set(fileDescriptor, calls);
      return calls === 1
        ? snapshot
        : ({
            ...snapshot,
            [changedField]: changedField === 'size' ? snapshot.size + 1 : 11,
          } as fs.Stats);
    });
    mockFs.readFileSync.mockImplementation((value: string) =>
      value === configPath ? configContents : dependencyContents,
    );
  ...
  it('should not read a structured dependency after its parent is swapped before stat', () => {
    let dependencyStats = 0;
    let parentSwapped = false;
    mockFs.realpathSync.mockImplementation((value: string) =>
      value === '/test/working/prompts/swapped.yaml' && parentSwapped
        ? '/private/tmp/outside/swapped.yaml'
        : value,
    );
    mockFs.statSync.mockImplementation((value: string) => {
      if (value === '/test/working/prompts/swapped.yaml') {
        dependencyStats++;
        if (dependencyStats >= 2) {
          parentSwapped = true;
        }
      }
      return {
...
`);
    ...
    expect(deps).toEqual([
      ...
      'graders/default.py',
      'graders/default-vars.js',
      'graders/default-rubric.txt',
      'graders/test.py',
      'graders/test-transform.js',
      'graders/postprocess.js',
      'graders/test-assert.py',
      './',
    ]);
  });

  it('should track typed grading-provider maps from default and external tests', () => {
    const files = new Map([
      [
        '/test/working/promptfooconfig.yaml',
        `defaultTest:\n  options:\n    provider:\n      text: file://graders/default.js\n      embedding:\n        id: python:graders/embed.py:call_api\ntests: file://tests/case.yaml`,
      ],
      [
        '/test/working/tests/case.yaml',
        `options:\n  provider:\n    text: file://graders/external.js\n    classification: ruby:graders/classify.rb:call_api`,
      ],
    ]);
    mockFs.readFileSync.mockImplementation((value: string) => {
      const contents = files.get(value);
    ...
    expect(deps).toEqual([
      ...
      'graders/embed.py',
      'tests/case.yaml',
      'tests/graders/external.js',
      'tests/graders/classify.rb',
      './',
    ]);
  });

  it('should track inline provider-specific option file dependencies', () => {
    mockFs.readFileSync.mockReturnValue(`
defaultTest:
  options:
    response_format:
      json_schema:
        schema: file://schemas/default-response.json
tests:
  - options:
      response_format: file://schemas/math-response.json
      output_format:
        schema: file://schemas/claude-response.json
      custom:
        nested:
          - file://schemas/provider-specific.json
          - null
          - 42
`);
    ...
    expect(deps).toEqual([
      'schemas/default-response.json',
      'schemas/math-response.json',
      'schemas/claude-response.json',
      'schemas/provider-specific.json',
    ]);
  });

  it('should expand runtime-backed response and output format globs from inline test options', async () => {
    const realGlob = await vi.importActual<typeof import('glob')>('glob');
    mockFs.readFileSync.mockReturnValue(`
tests:
  - options:
      response_format:
        type: json_schema
        schema: file://schemas/{one,two}/*.json
      output_format: file://schemas/*.json
`);
    mockGlob.hasMagic.mockImplementation(realGlob.hasMagic);
    mockGlob.sync.mockImplementation((pattern: string) => {
      if (pattern.endsWith('/schemas/one/*.json')) {
        return ['/test/working/schemas/one/response.json'];
      }
      if (pattern.endsWith('/schemas/two/*.json')) {
        return ['/test/working/schemas/two/response.json'];
      }
      if (pattern.endsWith('/schemas/*.json')) {
        return ['/test/working/schemas/output.json'];
      }
      return [];
    });
    ...
    expect(deps).toEqual([
      'schemas/output.json',
      'schemas/',
      'schemas/one/response.json',
      'schemas/two/response.json',
      'schemas/one/',
      'schemas/two/',
    ]);
    expect(core.warning).not.toHaveBeenCalled();
  ...
  it('should expand aliased response-format globs even when first encountered outside the format subtree', async () => {
    const realGlob = await vi.importActual<typeof import('glob')>('glob');
    mockFs.readFileSync.mockReturnValue(`
tests:
  - options:
      other: &shared-response
        schema: file://schemas/response/*.json
      response_format: *shared-response
      output_format: &shared-output
        schema: file://schemas/output/*.json
      otherOutput: *shared-output
`);
    mockGlob.hasMagic.mockImplementation(realGlob.hasMagic);
    mockGlob.sync.mockImplementation((pattern: string) =>
      pattern.endsWith('/schemas/response/*.json')
        ? ['/test/working/schemas/response/schema.json']
        : pattern.endsWith('/schemas/output/*.json')
          ? ['/test/working/schemas/output/schema.json']
          : [],
    );
  ...
  it('should preserve literal HTTP option and scenario paths without running glob preflight', async () => {
    const realGlob = await vi.importActual<typeof import('glob')>('glob');
    mockFs.readFileSync.mockReturnValue(`
tests:
  - options:
      response_format: 'file://schemas\\{one,two}.json'
      provider:
        id: https
        config:
          request: 'file://requests\\payload.txt'
          validateStatus: 'file://validators\\STATUS\\{one,two}.JS:validate'
          transformRequest: 'file://{{ env.REQUEST_TRANSFORM }}'
          transformResponse: 'file://{% if env.USE_RESPONSE %}response.JS{% endif %}'
          responseParser: 'file://transforms\\*.TS:parse'
          sessionParser: 'file://transforms\\[literal].JS:session'
          auth:
            type: file
            path: 'file://auth/token\\{literal}.py:get_auth'
scenarios:
      ...
      providers:
        - id: https
          config:
            ...
            auth:
              type: file
              path: 'file://auth\\{one,two}.py:get_auth'
`);
    mockGlob.hasMagic.mockImplementation(realGlob.hasMagic);
    ...
    expect(deps).toEqual([
      'requests\\payload.txt',
      'validators\\STATUS\\{one,two}.JS',
      'validators\\STATUS\\{one,two}.JS:validate',
      'transforms\\*.TS',
      'transforms\\*.TS:parse',
      'transforms\\[literal].JS',
      'transforms\\[literal].JS:session',
      'schemas\\{one,two}.json',
      'auth/token\\{literal}.py',
      './',
    ]);
    expect(mockGlob.sync).not.toHaveBeenCalled();
...
`);
  ...
  it('should fail closed for concatenated templated provider and transform paths', () => {
    mockFs.readFileSync.mockReturnValue(`
providers:
...
`);
  ...
  it('should resolve leading-slash Nunjucks filter paths under the config directory like Promptfoo', () => {
    mockFs.readFileSync.mockReturnValue(`
nunjucksFilters:
  rooted: /filters/rooted.js
`);
    ...
    expect(deps).toEqual(['evals/filters/rooted.js', './']);
    expect(core.warning).toHaveBeenCalledWith(
      expect.stringContaining('Watching the repository workspace'),
    );
  });

  it('should track commandLineOptions envPath files and comma-separated env paths', () => {
    mockFs.readFileSync.mockReturnValue(`
commandLineOptions:
  envPath:
    - null
    - ' , '
    - env/common.env
    - /test/working/shared/common.env
    - 'env/one.env, env/two.env, '
    - ../../outside.env
`);
    ...
    expect(core.warning).toHaveBeenCalledWith(
      expect.stringContaining(
        'config environment file dependency must stay within the checkout or config directory',
      ),
    );
  });
...
`);
  ...
  it('should extract nested file references from CSV and JSONL test files', () => {
    ...
    expect(deps).toEqual([
      ...
      'assets/jsonl.txt',
      'assets/jsonl-escaped.txt',
    ]);
  });

  it('should preserve quoted CSV commas, URLs, and literal parenthesis filenames while tracking nested refs', () => {
    const files = new Map([
      ['/test/working/promptfooconfig.yaml', 'tests: file://tests/cases.csv'],
      [
        '/test/working/tests/cases.csv',
        'url,expected\n"https://example.test/query?a=one,two",file://assets/expected(result).txt\n',
      ],
    ]);
    mockFs.readFileSync.mockImplementation((value: string) => {
      const contents = files.get(value);
    ...
    expect(deps).toEqual(['tests/cases.csv', 'assets/expected(result).txt']);
    expect(core.warning).not.toHaveBeenCalled();
  });

  it('should track typed file-backed CSV assertions without treating prose as a dependency', () => {
    const files = new Map([
      ['/test/working/promptfooconfig.yaml', 'tests: file://tests/cases.csv'],
      [
        '/test/working/tests/cases.csv',
        '__expected_result,__expected2,__expected3,__expected4,context\nequals:file://assets/expected(result).txt,javascript:file://validators/check.js:check,equals:literal,"See file://assets/not-a-dependency.txt",file://assets/direct.txt\n',
      ],
    ]);
    mockFs.readFileSync.mockImplementation((value: string) => {
      const contents = files.get(value);
    ...
    expect(deps).toEqual([
      'tests/cases.csv',
      'assets/expected(result).txt',
      'validators/check.js',
      'assets/direct.txt',
      './',
    ]);
    expect(deps).not.toContain('assets/not-a-dependency.txt');
  });

  it('should preserve embedded file URLs in CSV prose without running glob preflight', () => {
    const files = new Map([
      ['/test/working/promptfooconfig.yaml', 'tests: file://tests/cases.csv'],
      [
        '/test/working/tests/cases.csv',
        'context,expected\n"Use file://assets/{1..1000000000}.txt as an example",ok\n',
      ],
    ]);
    mockFs.readFileSync.mockImplementation((value: string) => {
      const contents = files.get(value);
    ...
    expect(deps).toEqual(['tests/cases.csv']);
    expect(mockGlob.hasMagic).not.toHaveBeenCalledWith(
      expect.stringContaining('{1..1000000000}'),
    );
    expect(mockGlob.sync).not.toHaveBeenCalled();
    expect(core.warning).not.toHaveBeenCalled();
  });
  ...
  it('should fail closed for generic Nunjucks expressions in structured dependencies while preserving local siblings', () => {
    const files = new Map([
      [
        '/test/working/promptfooconfig.yaml',
        'providers: file://providers/external.yaml\nprompts: file://prompts/external.yaml\ntests:\n  - file://tests/cases.csv\n  - file://tests/cases.jsonl',
      ],
      [
        '/test/working/providers/external.yaml',
        "note: '{{ env.PROVIDER_NOTE }}'\nasset: file://assets/provider.txt",
      ],
      [
        '/test/working/prompts/external.yaml',
        "raw: '{{ env.PROMPT_TEXT }}'\nasset: file://assets/prompt.txt",
      ],
      [
        '/test/working/tests/cases.csv',
        'context,expected\n"{{ env.CSV_CONTEXT }}",file://assets/csv.txt\n',
      ],
      [
        '/test/working/tests/cases.jsonl',
        `${JSON.stringify({ note: '{{ env.JSONL_NOTE }}', asset: 'file://assets/jsonl.txt' })}\n`,
      ],
    ]);
    mockFs.readFileSync.mockImplementation((value: string) => {
      const contents = files.get(value);
  ...
  - vars:
    ...
    assert:
      ...
      - type: ruby
        value: file://validators/check.RB:Validators::Format.check
      - type: not-ruby
        value: file://validators/inverse.rb:Validators::Format.check
      - type: contains
...
`);
  ...
  it('should extract direct file references from inline assertion value arrays', () => {
    mockFs.readFileSync.mockReturnValue(`
tests:
  - assert:
      - type: contains-any
        value:
          - file://expected/first.txt
          - 'file://expected/literal\\{one,two}.txt'
          - inline expected text
          - null
`);
...
`);
  ...
  it('should preserve local sibling dependencies beside a remote config reference', () => {
    mockFs.readFileSync.mockReturnValue(`
$ref: 'https://example.invalid/shared-config.yaml#/config'
providers:
  - file://providers/custom.js
prompts:
...
`);
  ...
  it('should cap warnings for many foreign Windows-absolute dependencies', () => {
    ...
    expect(deps).toEqual(['./']);
    expect(core.warning).toHaveBeenCalledTimes(12);
    expect(core.warning).toHaveBeenCalledWith(
      expect.stringContaining(
  ...
  it('should handle file read errors gracefully', () => {
    ...
    expect(
      (core.warning as unknown as Mock).mock.calls
        .map((call) => String(call[0]))
        .join('\n'),
    ).not.toContain('::error::forged-primary-read');
  });
...
`;
    ...
    mockFs.statSync.mockImplementation(
      (value: string) =>
        ({
          isDirectory: () => !value.endsWith('/promptfooconfig.yaml'),
          isFile: () => value.endsWith('/promptfooconfig.yaml'),
